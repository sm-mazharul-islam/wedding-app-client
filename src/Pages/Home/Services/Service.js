...
const Service = ({ service, isEventPage }) => {
  ...
  return (
    <div className="card w-auto bg-base-100 shadow-xl gap-2 rounded-none mx-auto">
      <figure className="px-4 pt-4 lg:px-8 lg:pt-8">
        <img src={image} className="rounded-none" alt={name} />
      </figure>
      <div className="card-body items-center text-center">
        ...
        {/* LOGIC: Change link and text based on the page */}
        <Link
          to={isEventPage ? `/details/${_id}` : "/eventCoordination"}
          className="t2 font-bold"
        >
          {isEventPage ? "Details" : "Choose Package"}
        </Link>
